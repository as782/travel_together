...
const { list: groupList, containerProps: containerGroupList, wrapperProps: wrapGroupList } = useVirtualList(groupCardDataList, {
    itemHeight: 300,
    overscan: 2,
})
const { list: userList, containerProps: containerUserList, wrapperProps: wrapUserList } = useVirtualList(userCardDataList, {
    itemHeight: 100,
    overscan: 2,
})
const { list: momentList, containerProps: containerMomentList, wrapperProps: wrapMomentList } = useVirtualList(momentCardDataList, {
    itemHeight: 400,
    overscan: 2,
})
...
<template>
    <main class="bg-white p-1 h-screen ">
        <van-nav-bar fixed :z-index="999" left-arrow @click-left="$router.back()">
            <template #title>
                <h4 class="text-sm">
                    关于： <span>{{ $route.query.key ? $route.query.key : '空' }}</span> 共 <em class="text-blue-300">{{
                        useSeach.dataConunt }}</em> 条结果
                </h4>
            </template>
        </van-nav-bar>
        <BlankSpaceBox :height="'50px'" />
        <van-tabs v-model:active="activeName" animated sticky>
            <van-tab title="组队" name="a">
                <van-empty v-if="!groupList.length" description="没有更多了"></van-empty>
                <div v-bind="containerGroupList" style="height: 100vh">
                    <div v-bind="wrapGroupList">
                        <template v-for="(cardinfo) in groupList " :key="cardinfo.data.card_id">
                            <GroupInfoCard @click="handleGotoDetal(cardinfo.data.card_id)" :cardData="cardinfo.data"
                                class="m-2" />
                        </template>
                    </div>
                </div>
            ...
            <van-tab title="动态" name="b">
                <van-empty v-if="!momentList.length" description="没有更多了"></van-empty>
                <div v-bind="containerMomentList" style="height: 100vh">
                    <div v-bind="wrapMomentList">
                        <template v-for="moment in momentList" :key="moment.card_id">
                            <MomentsActivityCard class="my-2" :moment-data="moment.data"
                                ...
                                @click-follow="() => handleClickFollow(moment.data.userInfo.user_id, moment.data.card_id)" />
                        </template>
                    </div>
                </div>
            </van-tab>
            <van-tab title="用户" name="c">
                <van-empty v-if="!userList.length" description="没有更多了"></van-empty>
