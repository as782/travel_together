  ...
  const publichList: any[] = res.data.list.map(async (item: any) => {
    ...
    if (item.post_id) {
      ...
      const isLike = likeUsers[item.dynamic_post_id].some(
        (item: any) => item.user_id === user_info?.user_id
      )
...
<template>
  <main>
      ...
      <div class="card-profile rounded-lg bg-white">
        <div class="profile-top flex flex-row-reverse w-full p-2">
          ...
          >
            <img
              ...
              :src="userInfoCardData?.user_info.avatar_url"
              alt="avatar"
            />
          </div>
        ...
        <div class="profile-center px-2">
          <div class="w-full">
            ...
            <template
              v-for="tag in userInfoCardData?.tags"
              :key="tag.tag_id"
            >
              <van-tag
                class="mx-1"
    ...
    <ChatAndFollowButton
      left-btn-type="primary"
      right-btn-type="success"
      :is-follow="isFollow"
      @left-btn-click="handleClickChat"
      @right-btn-click="handleClickFollow"
    />
  </main>
</template>
<style scoped></style>
