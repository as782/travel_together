...
const handleSearch = async () => {
    ...
    useSeach.searchResultData.status == 'succeeded' && router.push({
        path: '/searchresult',
        query: {
            key: searchKey.value
        }
    ...
    <main class="bg-white p-1">
        ...
        <div class="flex items-center m-2">
            ...
            <div class="flex-1 flex  mx-2 relative">
                ...
                <input type="text" ref="searchRef" class="w-full rounded-2xl p-1 pl-10 bg-blue-50" v-model="searchKey"
                    placeholder="搜索组队/动态/用户" />
            </div>

            <button class="flex-none w-8 p-1 rounded-md  bg-blue-300 shadow-xl shadow-orange-100 text-xs text-white" @click="handleSearch">搜索</button>
        ...
        <div class="">
            <div class="m-2">
                ...
                <ul class="flex w-full flex-wrap" @click="handleSelectTag">

                    <li class="m-2" v-for="key in historyKeys" :key="key">
                        <span class="w-full rounded-xl  p-1 px-2 bg-slate-400 bg-opacity-20 text-xs"
                            :data-tagname="key">{{ key }}</span>
                    </li>
