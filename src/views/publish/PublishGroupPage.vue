...
const getGroupPostDetail = async (postId: number) => {
  try {
    ...
    itineraryFileList.value = fileListFactory(
      [{ image_id: postId, image_url: itinerary }],
      'itinerary'
    )
  } catch (error) {
    console.error(error)
...
const onSubmit = async () => {
  try {
    ...
    deleteItineraryImgIds.value.length &&
      (await deleteImgForIds(deleteItineraryImgIds.value, 'itinerary'))
    ...
    >
      ...
      <van-cell-group class="p-1 m-2 border border-blue-200 rounded-md">
        ...
        <van-field
          ...
          v-model="formData.estimated_expense"
          type="text"
          label="预计消费:"
          placeholder="例如: 8 (元)"
        />
      ...
      <van-cell-group class="p-1 m-2 border border-blue-200 rounded-md">
        <van-field
          ...
          @click="
            () => {
              addressPopupState.showAddressPopup = true
              addressPopupState.flag = 0
            }
          "
          placeholder="例如: xxx"
        />
        <van-field
          ...
          @click="
            () => {
              addressPopupState.showAddressPopup = true
              addressPopupState.flag = 1
            }
          "
          placeholder="例如: xxx"
        />
        <van-popup
          v-model:show="addressPopupState.showAddressPopup"
          position="bottom"
        >
          <van-area
            title="标题"
