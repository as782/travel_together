...
const handleCommentOnLoad = () => {
  setTimeout(() => {
    commentList.value =
      groupPostDetailData.value?.comments
        ?.map((moment) => ({
  ...
  <main class="bg-blue-100">
    <van-config-provider>
        ...
        <div class="flex justify-around w-full my-2">
          <div class="flex flex-col justify-center items-center">
            ...
            <span class="text-sm text-blue-400 font-bold"
              >{{ groupPostDetailData?.team_size }}人</span
            >
          </div>
          <div class="flex flex-col justify-center items-center">
            <h4 class="text-xs">预计消费</h4>
            <span class="text-sm text-blue-400 font-bold"
              >{{ groupPostDetailData?.estimated_expense }}元</span
            >
          </div>
          <div class="flex flex-col justify-center items-center">
            ...
            <span class="text-sm text-blue-400 font-bold">
              {{
                groupPostDetailData?.gender_requirement == 'male'
                  ? '男'
                  : groupPostDetailData?.gender_requirement == 'female'
                    ? '女'
                    : '不限'
              }}
            </span>
          ...
          <div class="flex flex-col justify-center items-center">
            ...
            <span class="text-sm text-blue-400 font-bold">{{
              groupPostDetailData?.payment_method
            }}</span>
          </div>
        </div>
      </div>
      <div class="w-full p-2 my-3 bg-white">
        ...
        <div
          class="avator flex items-center w-11/12 h-14 mx-auto shadow-md shadow-gray-300 rounded-3xl"
        >
          <van-image
            round
            class="w-8 flex-none aspect-square mx-2"
            :src="postUserInfo?.avatar_url"
          />
          <div class="flex flex-1 flex-col">
            <span
              class="w-3/5 h-full mx-1 my-1 truncate text-sm text-nowrap text-gray-500 font-bold"
            >
          ...
          <van-icon
            ...
            color="#9d9dff"
            name="arrow"
            @click="() => toUserHome(postUserInfo?.user_id!)"
            size="30"
          />
        </div>
      </div>
      ...
      <div class="p-2 bg-white">
        ...
        <div class="w-full my-2">
          <ul
            class="flex p-1 overflow-x-auto"
            @click="handleUselistClick"
          >
            <template
              v-for="joinedUser in joinTeamUserList"
              :key="joinedUser.user_id"
            >
              <div
                class="flex flex-col mx-1 justify-center items-center"
                :data-user_id="joinedUser.user_id"
              >
                <img
                  class="w-8 aspect-square mx-2 rounded-full"
                  :src="joinedUser.avatar_url"
                  :data-user_id="joinedUser.user_id"
                />
                <span
      ...
      <div class="plan w-full p-2 mt-3 bg-white">
        ...
        <div class="img-content">
          <img
            v-if="groupPostDetailData?.itinerary"
            :src="groupPostDetailData?.itinerary"
            alt="行程安排"
          />
          <span
            v-else
      ...
      <div class="w-full p-2 mt-3 bg-white">
        ...
        <div class="active-comment flex justify-center items-center">
          <van-image
            round
            class="flex-none w-10 aspect-square mx-2"
            :src="useUser.userInfo?.avatar_url"
          />
          <input
      ...
      <van-action-bar>
        ...
        <van-action-bar-button
          ...
          color="#9d9dff"
          type="danger"
          :to="`/publishgroup/${groupPostDetailData?.post_id}`"
          text="去编辑"
        />
      </van-action-bar>
    </van-config-provider>
