...
watch(aboutMessage, () => {
  ...
  messageCardList.value = mergedMessages.map((message) =>
    generateMessageCardInfo(message, userStore.userInfo?.user_id!)
  )
  ...
  interactiveNoticeList.value = interactiveNotice.map((message: any) =>
    generateMessageCardInfo(message, userStore.userInfo?.user_id!)
  )
...
function mergeMessages(sentMessages: MessageContent[], receivedMessages: MessageContent[]) {
  ...
  receivedGroups.forEach((messages, userId) => {
    ...
    if (existingMessage) {
      mergedMessages.set(
        userId,
        existingMessage.created_at > latestMessage.created_at ? existingMessage : latestMessage
      )
    } else {
      mergedMessages.set(userId, latestMessage)
    }
  ...
  <main class="bg-white">
    <van-sticky :offset-top="50">
      ...
      >
        ...
        >
          <template
            v-for="adminNotice in adminNoticeList"
            :key="adminNotice.id"
          >
            <van-swipe-item class="w-full flex items-center">
              ...
              >
                ...
                <div class="flex justify-center items-center">
                  <span class="text-xs text-blue-500 mx-2"
                    >------{{ adminNotice.userInfo.nickname }}</span
                  >
                  <van-image
                    class="flex-none w-5 aspect-square shadow-lg shadow-slate-400"
                    round
                    fit="cover"
                    :src="adminNotice.userInfo.avatar_url"
                  />
                </div>
              </div>
            </van-swipe-item>
    ...
    >
      <div
        class="system-info flex p-2 m-2 border shadow-md"
        @click="router.push('/aboutmoment')"
      >
        <van-icon
          name="bell"
          class="flex-none"
        ...
        <div class="mx-2 flex-1">
          ...
          <p
            v-if="!interactiveNoticeList.length"
            class="text-xs truncate text-left text-gray-500"
          >
            无动态
          </p>
          <p
            v-if="interactiveNoticeList[0]?.type === MessageType.TEAM_ACTIVITY_POST_COMMENT"
            class="text-xs truncate text-left text-gray-500"
          >
            <span class="text-blue-300">{{ interactiveNoticeList[0]?.userInfo?.nickname }}</span>
            评论了你的组队：{{ interactiveNoticeList[0]?.content }}
          </p>
          <p
            v-if="interactiveNoticeList[0]?.type === MessageType.DYNAMIC_POST_COMMENT"
            class="text-xs truncate text-left text-gray-500"
          >
            <span class="text-blue-300">{{ interactiveNoticeList[0]?.userInfo?.nickname }}</span>
            评论了你的动态：{{ interactiveNoticeList[0]?.content }}
          </p>
          <p
            v-if="interactiveNoticeList[0]?.type === MessageType.TEAM_ACTIVITY_POST_LIKE"
            class="text-xs truncate text-left text-gray-500"
          >
            <span class="text-blue-300">{{ interactiveNoticeList[0]?.userInfo?.nickname }}</span>
            点赞了你的组队
          ...
          <p
            v-if="interactiveNoticeList[0]?.type === MessageType.DYNAMIC_POST_LIKE"
            class="text-xs truncate text-left text-gray-500"
          >
            <span class="text-blue-300">{{ interactiveNoticeList[0]?.userInfo?.nickname }}</span>
            点赞了你的动态
          ...
          <p
            v-if="interactiveNoticeList[0]?.type === MessageType.FOLLOW_NOTIFICATION"
            class="text-xs truncate text-left text-gray-500"
          >
            <span class="text-blue-300">{{ interactiveNoticeList[0]?.userInfo?.nickname }}</span>
            关注了你
