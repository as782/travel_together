...
<template>
  <div class="moment-card p-2 rounded-md bg-white shadow-md shadow-gray-300">
    <div class="card-top flex items-center">
      <van-image
        class="flex-none w-8 aspect-square"
        round
        fit="cover"
        :src="momentData?.userInfo?.avatar_url"
      />
      <div class="flex flex-col flex-1 ml-2">
        <h4 class="text-sm">{{ momentData?.userInfo?.nickname }}</h4>
        <span class="userInfo-state text-xs text-gray-500"
          >{{ momentData?.createTime.slice(0, 10) }}
      ...
      <van-button
        class="flex-none"
        @click.stop="$emit('clickFollow')"
        :icon="momentData?.isFollow ? '' : 'plus'"
        type="primary"
        round
        :plain="momentData?.isFollow"
        size="small"
      >
    ...
    <div class="card-content py-1">
      <div class="content-text">
        <van-text-ellipsis
          :content="momentData?.content?.desc"
          rows="2"
          expand-text="展开"
          @click.stop
          collapse-text="收起"
      ...
      <div class="content-resource my-2 columns-2 flex-wrap">
        <template
          v-for="img in momentData?.content?.images"
          :key="img.image_id"
        >
          <van-image
