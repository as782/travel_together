...
watch(myLikeGroupPostDetailList, async (newValue) => {
    groupCardData.value = await Promise.all(newValue.data.map(async (item) => {
...
<template>
    <div class="bg-white">
        <van-tabs v-model:active="activeName" animated sticky :lazy-render="false" :offset-top="50">
            <van-tab name="group" title="组队">
                <div class="p-2">
                    <template v-for="cardinfo in groupCardData " :key="cardinfo.card_id">
                        <GroupInfoCard @click="handeGroupCardClick(cardinfo.card_id)" :cardData="cardinfo"
                            class="my-4" />
                    </template>
                </div>
            </van-tab>
            <van-tab name="moment" title="动态">
                <div class="p-2">
                    <template v-for="moment in momentCardData" :key="moment.id">
