...
const getCardDataList = async (params: PageParams) => {
  try {
    ...
    result.data.list = await Promise.all(
      result.data.list.map(async (item: any) => {
...
<template>
  <main class="bg-white">
    ...
    >
      ...
      >
        <GroupInfoCard
          @click="handleGotoDetal(cardinfo.card_id)"
          :cardData="cardinfo"
          class="m-2"
        />
      </template>
