...
/** 自己信息 */
const userInfo = computed(() => ({ user_id: useUser.userInfo?.user_id, nickname: useUser.userInfo?.nickname, avatar_url: useUser.userInfo?.avatar_url }));
// 标题是否显示
...
const getChatRecord = async () => {
    if (pageState.value.total !== -1 && (pageState.value.currentPage - 1) * pageState.value.pageSize >= pageState.value.total) {
        showToast("没有更多消息了");
    ...
    try {
        ...
        chatRecordList.value.unshift(...result.data.list.map((item: GetMessagesParamsResponse) => {
        ...
        <van-pull-refresh class="flex-1" v-model="loading" @refresh="onRefresh">
            <div>
                <template v-for="chatRecord in chatRecordList" :key="chatRecord.id">
                    <MessageCard :is-send="chatRecord.isSender" :content="chatRecord.content"
                        :name="chatRecord.user_info.nickname" :avatar="chatRecord.user_info.avatar_url"
